...
function plot_events(data::Matrix{T}, events::Vector{Int64};fs=30_000.0) where T <: Real
    ...
    #dummy line just to have something that we can modify later
    plot_zoom!(zscene, data[2,:], fs=fs)
    lines!(zscene.scene.children[2], fill(0.0, 10), range(0.0, stop=0.1,length=10),color="red")
    ...
    map(fscene.selected_point) do sp
        if 0 < sp <= size(data,2)
            ...
            push!(zscene.scene.children[2].plots[zscene.nlines+2][1],[Point2f0(t0,_y) for _y in range(ymin,stop=ymax,length=10)])
        end
    end
